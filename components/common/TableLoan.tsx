...
const column = ['ID', 'Elemento', 'Estado', 'Usuario', 'Fecha de préstamo', 'Fecha de devolución', 'Acciones'];
...
const TableLoan = () => {
	...
	const currentItems = loan.slice(indexOfFirstItem, indexOfLastItem);
							...
							<div className='overflow-hidden border border-gray-200 md:rounded-lg'>
								<Table column={column}>
									{currentItems.map((item, index) => (
										<tr key={item.id}>
											<TableRowLoan loan={item}></TableRowLoan>
										</tr>
									))}
								</Table>
							</div>
